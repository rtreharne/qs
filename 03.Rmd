...
- **Linear range**: the calibration curve is only valid within a certain concentration range. At very high concentrations, the relationship bay no longer be linear (due to factors like light scattering). Extrapolating concentrations from our calibration curves that are beyond those of our known standards is very, very naughty. Don't do it!
- **Reproducibility**: It is critical that same instrument (including scan settings and wavelength) be use for both the standards and unknowns.
...
There's something not right about this dataset. It's sideways! The columns of data run horizontally instead of vertically. We don't like this, it make it much harder to plot figures and perform any analysis.
...
```{r calibration-curve, echo=FALSE, fig.cap="Calibration curve for Haemoglobin determined from a standard set generated from a 1:1 serial dilution from a starting 1.5 mg/mL solution. Values of m=0.3827 and b=0.0024 were extracted from the linear relationship A = m * C + b, where b is the systematic error associated with the measurement (detector noise).", out.width="100%"}
knitr::include_graphics("img/03/calibration_curve.png")
```
## Linear Regression

A linear trendline is visually helpful, but what we’re actually doing is something more powerful: **linear regression**.
...
In the BIOS103 Canvas course you will find this week's **formative** and **summative** assignments. 
   ...
   + You have until 5pm on the day of the workshop for this week's content to submit your **summative** assignment. There will be no opportunity to submit late and you will receive a zero score for the week if you do not submit.
